...
auto initialize_vector(Type value, B variable){
    return std::vector<Type>(variable, value);
}

template<typename Type, typename B, typename... T>
auto initialize_vector(Type value, B variable, T... dim1){
    auto answer = initialize_vector(value, dim1...);
    return std::vector<decltype(answer)>(variable, answer);
}
